...
ov::OutputVector create_loop_for_tf_while(const std::string& while_node_name,
    ...
    for (size_t input_ind = 0; input_ind < body_condition_output_idx; ++input_ind) {
        std::cout << "[EMUTEX DEBUG] [create_loop_for_tf_while] set_merged_input body_params[input_ind] " << body_params[input_ind]->get_friendly_name() << std::endl;
        if (has_parent_parameter(body_results[input_ind])) {
            invariant_input_indexes.push_back(input_ind);
            continue;
        }
        // check body_results[input_ind] body_params[input_ind] ?
        // if not connected, add set_merged
        // if connected push_back to invariant_input_indexes
    ...
    for (size_t idx : invariant_input_indexes) {
        std::cout << "[EMUTEX DEBUG] [create_loop_for_tf_while] set_invariant_input body_params[input_ind] " << body_params[idx]->get_friendly_name() << std::endl;
    ...
    for (size_t input_ind = body_condition_output_idx; input_ind < input_size; ++input_ind) {
        std::cout << "[EMUTEX DEBUG] [create_loop_for_tf_while] set_invariant_input body_params[input_ind] " << body_params[input_ind]->get_friendly_name() << std::endl;
    ...
    for (size_t output_ind = 0; output_ind < body_condition_output_idx; ++output_ind) {
        // if body_results[output_ind] - connected with body_params[???], do not do the next line
        // as_type_ptr<Parameter>(body_results[output_ind])
        //loop_outputs.push_back(loop->input_value(/* description->input_index */ ));
        ...
        loop_outputs.push_back(loop->get_iter_value(body_results[output_ind]));
    }
    loop->validate_and_infer_types();

    {
        const int num_subgraphs = static_cast<int>(loop->get_internal_subgraphs_size());
        std::cout << "[EMUTEX DEBUG] [create_loop_for_tf_while] num_subgraphs " << num_subgraphs << std::endl;
        const auto& descriptions = loop->get_input_descriptions();
        for (const auto& desc : descriptions) {
            if(!std::dynamic_pointer_cast<ov::op::util::MultiSubGraphOp::InvariantInputDescription>(desc)) {
                std::cout << "[EMUTEX DEBUG] is NOT MultiSubGraphOp::InvariantInputDescription" << std::endl;
            }
        }
    }
